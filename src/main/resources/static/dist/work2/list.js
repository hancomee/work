...
!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(22), __webpack_require__(27), __webpack_require__(24), __webpack_require__(4), __webpack_require__(25), __webpack_require__(16), __webpack_require__(23)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (require, exports, Search_1, _util_1, _template_1, _events_1, Pager_1, _remap_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var __$attrEvent = _events_1.Events.__$attrEvent;
    var $state = '작업대기 시안검토 시안완료 작업중 입고 납품 완료'.split(' '), _a = Array.prototype, forEach = _a.forEach, indexOf = _a.indexOf;
    ...
    var Info = /** @class */ (function () {
        function Info(element, navTable, search) {
            this.element = element;
            this.navTable = navTable;
            this.search = search;
            this.pager = new Pager_1.Pager(element, 5, 4);
        }
        Info.prototype.apply = function () {
            this.name = $state[this.search.state];
            return this;
        };
        Info = __decorate([
            _template_1.Template('&info', false),
            __metadata("design:paramtypes", [Object, Object, ListManager])
        ], Info);
        return Info;
    }());
    var StateList = /** @class */ (function () {
        function StateList(element, data, search) {
            this.element = element;
            this.data = data;
            this.search = search;
        }
        StateList.prototype.apply = function (data) {
            var index = (this.data = data).index, state = this.search.state;
            if (index === state)
        ...
        StateList = __decorate([
            _template_1.Template('&stateList'),
            __metadata("design:paramtypes", [Object, Object, ListManager])
        ], StateList);
        return StateList;
    }());
    var ItemList = /** @class */ (function () {
        function ItemList(element, data, search) {
            this.element = element;
            this.data = data;
            this.search = search;
        }
        ItemList.prototype.apply = function () {
            return this;
        };
        ItemList = __decorate([
            _template_1.Template('&itemList'),
            __metadata("design:paramtypes", [Object, Object, ListManager])
        ], ItemList);
        return ItemList;
    }());
    ...
    var WorkCreator = /** @class */ (function () {
        function WorkCreator(element, customer, title, resultList) {
            var _this = this;
            this.element = element;
            ...
            // 화면 끄기
            element.addEventListener('mousedown', function (e) { return e.target === element && _this.off(); });
            var keyword;
            var classList = element.classList, __getNames = function (val) {
                if (val === void 0) { val = customer.value; }
                fetch('/work/db/customer/' + encodeURIComponent(val)).then(function (res) { return res.json(); }).then(function (list) {
                    if (val === keyword) {
                        var creatable_1 = true;
                        resultList.innerHTML = list.map(function (data, i) {
                            creatable_1 && (creatable_1 = !(data.name === val));
                            return '<li data-id="' + data.id + '" data-name="' + data.name + '"><span>' + data.name + '</span></li>';
                        }).join('');
                        creatable_1 ? classList.add('creatable') : classList.remove('creatable');
                    }
                });
            }, $creator = {
                customer: function () {
                    _util_1.$post('/work/db/customer', { name: customer.value }).then(function (res) { return __getNames(); });
                },
                work: function () {
                    _util_1.$post('/work/db/create', { 'customer_id': element.getAttribute('data-customer'), title: title.value })
                        .then(function (res) { return res.json(); })
                        .then(function (uuid) { return location.href = '/work/view/' + uuid; });
                }
            };
            // 버튼 클릭
            element.addEventListener('click', function (e) {
                var func = $creator[e.target.getAttribute('data-creator')];
                func && func();
            });
            // 고객이름 입력
            customer.addEventListener('keyup', function (e) {
                var value = customer.value = customer.value.trim(), key = e.key;
                if (!value) {
                ...
                // 리스트 선택
                if (key === 'Enter') {
                    var target = resultList.getElementsByClassName('selected')[0];
                    if (target) {
                        _this.setCustomer(target.getAttribute('data-id'), target.getAttribute('data-name'));
                        keyword = '';
                    }
                }
                // 리스트 이동
                else if (key.indexOf('Arrow') === 0) {
                    var list = resultList.getElementsByTagName('li'), selected = resultList.getElementsByClassName('selected')[0], target = void 0, len = list.length, index = indexOf.call(list, selected);
                    if (key === 'ArrowDown') {
                        if (index === -1) {
                            if (len)
                                target = list[0];
        ...
        WorkCreator.prototype.setCustomer = function (id, name) {
        ...
        WorkCreator.prototype.reset = function () {
            this.customer.value = this.title.value = this.resultList.textContent = '';
            this.element.removeAttribute('data-customer');
            this.element.classList.remove('creatable');
            return this;
        };
        WorkCreator.prototype.apply = function () {
            return this;
        };
        WorkCreator = __decorate([
            _template_1.Template('&workCreator', false),
            __metadata("design:paramtypes", [Object, Object, Object, Object])
        ], WorkCreator);
        return WorkCreator;
    }());
    (function () {
        var search = new ListManager().resetHash(), $stateElement = document.getElementById('state'), $container = document.getElementById('container'), 
        // 엔터로 동작하기
        $search = (function (input) {
            ...
            return input;
        })(document.getElementById('search')), workCreator = WorkCreator.$create(), info = Info.$create({ search: search }).apply(), stateList = $state.map(function () { return StateList.$create({ search: search }); }), 
        // 데이터 가져오기
        __render = function () {
            _util_1.$post('/work/list?' + search.reset().toString(), null).then(function (res) { return res.json(); }).then(function (values) {
                var contents = values.contents, price = values.price, count = values.count, today = values.today, page = values.page, totalPages = values.totalPages;
                // 리스트 갱신
                stateList.forEach(function (v, i) { return v.apply({
                    index: i,
                    name: $state[i],
                    count: count[i],
                    price: price[i],
                ...
                info.apply();
                $stateElement.appendChild(_util_1.__appendFragment(stateList));
                $container.textContent = '';
                $container.appendChild(_util_1.__appendFragment(contents.map(function (value) { return ItemList.$create({ data: value }).apply(); })));
            });
        };
        //
        var keyEvent = _remap_1.__remap({
            Enter: function () {
                workCreator.element.classList.contains('on') || workCreator.on();
            }
        });
        document.addEventListener('keyup', function (e) { return e.target === document.body && keyEvent[e.key] && keyEvent[e.key](e); });
        __$attrEvent(document.body, 'click', 'data-event', /** @class */ (function () {
            ...
            pageMove: function (_a) {
                var page = _a.page;
                page && search.writeHash({ page: page });
            },
            changeState: function (_a) {
                var state = _a.state, id = _a.id;
                _util_1.$post('/work/db/update/state/' + id + '/' + state, null).then(function (res) {
                    __render();
                });
            },
            search: function () {
                var value = $search.value = $search.value.replace(/\s/g, '');
                if (value)
                    search.writeHash({ search: value, page: 1 });
                else
                    search.writeHash({ search: null });
            },
            createWork: function () {
                workCreator.on();
            },
            removeWork: function (_a) {
                var itemList = _a.itemList;
                _util_1.$delete('/work/db/remove/' + itemList.data.id).then(function (res) {
                    __render();
                });
            }
        });
