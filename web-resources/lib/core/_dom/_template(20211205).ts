...
const
    ...
    DEFAULT_DIRECTIVE = {
        ...
        $toggle(ele: HTMLElement, bean, values) {
            __toggleClass(ele, values, !!__read(values[2], bean));
        },
        /*
        ...
        $template(ele: HTMLElement, bean, [prop, templateName, noClone]) {
            ...
            if (data && template) {
                ...
                ele.appendChild(__reduceFragment(data, (v) =>
                    __apply(noClone ? template : template.cloneNode(true) as any, v)));
            }
        },
    ...
    __apply = (function () {
        ...
        function $$(ele: HTMLElement, bean) {
            ...
            while (i-- > 0) {
                if (children[i].hasAttribute('_template'))
                    continue;

                if (v = (e = children[i]).getAttribute('_access')) {
                    __access(e, bean, v);
                }

                children[i].hasAttribute('_ignore') || $$(e, bean);
            }
            return ele;
        }

        return (ele: HTMLElement, bean = DUMMY) => {
