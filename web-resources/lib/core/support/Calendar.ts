...
export namespace Calendar {
    ...
    export function create(v1?, v2?, v3?, v4?, v5?, v6?) {
        ...
        if (typeof v6 === 'number') v = new Date(v1, v2, v3, v4, v5, v6);
        else if (typeof v3 === 'number') v = new Date(v1, v2, v3);
    ...
    // 주차는 0부터
    export function dateFromWeek(y: number, m: number, w: number): Date[] {
        let date = new Date(y, m, 1);
        date = new Date(y, m, (date.getDate() * -1) + (w * 7) + 1);

        let result = [], d = date.getDate(),
            i = 0, l = 7;
    ...
    // 날짜 계산
    export function days(before: Date, after = new Date()) {
