...
export type EACH_HANDLER = <T>(inputs: INPUTS[], p: string, INPUT_MAP, group: HTMLElement, t: T) => void;
export type EACH_HANDLER_MAP = { [index: string]: EACH_HANDLER };
...
export class Forms {
    ...
    each<T>(directive: EACH_HANDLER_MAP, obj?: T): T
    each<T>(handler: EACH_HANDLER, obj?: T): T
    each(handler, obj?) {

        if (typeof handler === 'function') {
            this.groups.forEach(g => {
                let p, {inputs, element} = g;
                for (p in inputs)
